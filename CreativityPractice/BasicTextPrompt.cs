...
namespace CreativityPractice
{
    public class BasicTextPrompt
    {
        ...
        public static string checkIfOverTimeLimit(string promptString, int timeLimit)
        {
            ...
            // if prompt doesn't have a time, then return an error
            if (!promptString.Contains("time"))
            {
                return "ERROR";
            }
            ...
            Console.WriteLine("Checking time limit");
            ...
            System.Windows.Forms.MessageBox.Show("tim = " + tim);
            if (tim > timeLimit)
            {
                return "ERROR";
            }
